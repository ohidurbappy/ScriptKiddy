...
if __name__ == "__main__":
    ...
    for pdf_file in Path('./pdf').glob('*.pdf'):
    ...
    # configure presentation aspect ratio
    page = doc.loadPage(0)
    aspect_ratio = page.rect.width / (page.rect.height/2)
    prs.slide_width = int(prs.slide_height * aspect_ratio)
    ...
    for pdf_file in pdf_files:
        ...
        for page_no in trange(0, page_count):
            ...
            # divide the image into 2 parts
            im=Image.open(image_file)
            width, height = im.size
            top_im=im.crop((0,0,width,height/2))
            bottom_im=im.crop((0,height/2,width,height))

            # convert pillow to file object
            top_im_file = io.BytesIO()
            bottom_im_file = io.BytesIO()
            top_im.save(top_im_file, format='PNG')
            bottom_im.save(bottom_im_file, format='PNG')


            # add a slide
            slide = prs.slides.add_slide(blank_slide_layout)    
            left = top = Cm(0)
            slide.shapes.add_picture(top_im_file, left, top, height=prs.slide_height)

            slide = prs.slides.add_slide(blank_slide_layout)    
            left = top = Cm(0)
            slide.shapes.add_picture(bottom_im_file, left, top, height=prs.slide_height)

    output_file = "output2.pptx"
